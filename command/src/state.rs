...
mod tests {
  use super::*;
  use messages::{Order,HttpFront,Instance,BackendProtocol};
  ...
  fn diff() {
    ...
    state.handle_order(&Order::AddInstance(Instance { app_id: String::from("app_1"), ip_address: String::from("127.0.0.2"), port: 1027 }));
    state.handle_order(&Order::AddInstance(Instance { app_id: String::from("app_2"), ip_address: String::from("192.167.1.2"), port: 1026 }));
    state.handle_order(&Order::AddApplication(Application { app_id: String::from("app_2"), sticky_session: true, backend_protocol: BackendProtocol::TCP }));
    ...
    state2.handle_order(&Order::AddApplication(Application { app_id: String::from("app_3"), sticky_session: false, backend_protocol: BackendProtocol::TCP }));
   ...
   let e = vec!(
     ...
     Order::AddApplication(Application { app_id: String::from("app_3"), sticky_session: false, backend_protocol: BackendProtocol::TCP }),
   );
